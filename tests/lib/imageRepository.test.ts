...
describe('ImageRepository', () => {
	itWithFixtures('should be able to save an image', async ({ savedUser, imageRepository, image }) => {
	...
	itWithFixtures(
		...
		async ({ savedUser, imageRepository, image }) => {
			...
			expect(imageRepository.upsertImage(savedUser.id, 0, image)).rejects.toThrow(ImageRepositoryError);
		}
	);
	...
	itWithFixtures(
		...
		async ({ savedUser, imageRepository , image}) => {
			expect(imageRepository.upsertImage(savedUser.id, 5, image)).rejects.toThrow(ImageRepositoryError);
		}
	);

	itWithFixtures('should be able to delete a picture', async ({ savedUser, imageRepository, image}) => {
		await imageRepository.upsertImage(savedUser.id, 0, image);
